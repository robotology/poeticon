...
void* threadFn_tgso(void *arg)
{
	struct threadArg* tmpArg = (struct threadArg *)arg;
	//call tgso function here!
	if(tmpArg->tmap == NULL)
    {
		printf("\n tmap not loaded properly..");
		exit(1);
	}
	CvMat* tmp= tgso(*tmpArg->tmap,tmpArg->ntex,tmpArg->sigma,tmpArg->ori,*tmpArg->tsim,tmpArg->useChi2);

	if (tmp == NULL)
    {
		printf("\n Inside thread Fn: thread %d did not assign the pointer right",tmpArg->thread_num);
		exit(1);
	}
	tmpArg->tg = tmp;
...
CvMat **tgmo (CvMat &tmap, int ntex, double sigma, double *&theta, int norient, CvMat &tsim, int useChi2)
{
	...
	struct threadArg** tmpArg;
	tmpArg  = (struct threadArg **)malloc(norient*sizeof(struct threadArg*));

	for (int i=0;i<norient;i++)
    {
		theta[i]  = (double)i/norient*M_PI;
		tmpArg[i] = (struct threadArg*) malloc(sizeof(struct threadArg));
		tmpArg[i]->tmap       = &tmap;
		tmpArg[i]->ntex       = ntex;
		tmpArg[i]->sigma      = sigma;
		tmpArg[i]->ori        = theta[i];
		tmpArg[i]->tsim       = &tsim;
		tmpArg[i]->useChi2    = useChi2;
	...
	for(int i=0; i<norient; i++)
    {
		...
		if (tmpArg[i]->tg == NULL)
        {
			printf("\n outside thread Fn: thead %d did not assign the pointer right",i);
			exit(1);
        }
...
CvMat **cgmo (IplImage* im, int norient, double* theta)
{
	...
	if (im->nChannels == 3)
    {
		...
		cvCvtColor(temp,temp,CV_BGR2Lab);
