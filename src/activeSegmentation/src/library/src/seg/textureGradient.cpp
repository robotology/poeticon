...
void assignTextons(CvMat* tmap, CvMat* grayImg, Property &fileLoc){
	//char filterBin_dirLoc[1000];
	findWorkDir();
    ...
    string texture13=fileLoc.find("texture_13").asString();
    string texture19=fileLoc.find("texture_19").asString();
...
CvMat** detTG(IplImage* im, int norient, double* gtheta, Property &fileLoc){
	...
	assignTextons(tmap, imgGrayMat, fileLoc);
