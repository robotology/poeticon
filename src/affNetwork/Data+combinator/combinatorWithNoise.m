...
function [ output_args ] = combinatorWithNoise( )
%function [ output_args ] = combinatorWithNoise( fileDesc, fileEff, fileAff )
%[ output_args ] = combinator( fileDesc, fileEff, fileAff )

 fileDesc = 'descDataMin_4objs_10views_NEW.txt';
 fileEff = 'effData_TRAIN_balanced.txt';
 fileAff = 'affData_03_2015_descEffNoise_25series_good.txt';
...
for n = 1:size(eff{1,1},1)
    ...
    for Tool = 1:size(indTool,1)
        for Target = 1:size(indTarget,1)
            ...
            temp_aff(Target+(Tool-1)*size(indTarget,1),12) = (eff{1,25}(n) - eff{1,5}(n)) + ( rand*noiseRange*2 - noiseRange); %effectX + uniform noise
            temp_aff(Target+(Tool-1)*size(indTarget,1),13) = (eff{1,26}(n) - eff{1,6}(n)) + ( rand*noiseRange*2 - noiseRange); %effectY + uniform noise
