...
{
    // draws a CvBox2D, filled - http://stackoverflow.com/questions/9082204/opencv-draw-a-white-filled-polygon
...
{
    // draws a CvBox2D, borders only (non-filled)
    CvPoint2D32f pointsf[4];
    cvBoxPoints( box , pointsf );
    ...
    cvLine(img,pointsi[0],pointsi[1],CV_RGB(255,255,255),thickness,CV_AA,0);
    cvLine(img,pointsi[1],pointsi[2],CV_RGB(255,255,255),thickness,CV_AA,0);
    cvLine(img,pointsi[2],pointsi[3],CV_RGB(255,255,255),thickness,CV_AA,0);
    cvLine(img,pointsi[3],pointsi[0],CV_RGB(255,255,255),thickness,CV_AA,0);
}
